/**
 * @fileoverview 儲存草稿 / 匯出時自動更新副本受文者。
 *
 * ─── 副本處理流程（autoFillRecipients）────────────────────────
 *
 * 前提：主旨含「議員」關鍵字。
 *
 * 步驟 1（等議員提案偵測）：
 *   若主旨含「等議員提案」且佔位符尚未填寫（[○〇]+），
 *   由 app.js 的「提案議員名單」modal 負責處理，本函式直接 skip。
 *
 * 步驟 2（擷取議員姓名）：
 *   以名冊最長姓名為 N，掃描「議員」關鍵字前 N 個中文字，
 *   確認包含名冊中的合法姓名。
 *
 * 步驟 3（擷取組別）：
 *   若主旨無組別（第N組）→ 執行步驟 5（單一議員模式）。
 *   若主旨有組別 → 執行步驟 4（整組模式）。
 *
 * 步驟 4（整組模式）：
 *   偵測會議 / 部門類型 → 驗證議員組別正確 → 移除所有 OOO 佔位議員、
 *   新增主旨議員（第一位）及同組其他議員，保留非議員受文者。
 *
 * 步驟 5（單一議員模式）：
 *   移除未確認的佔位議員（不在名冊者），保留已確認的其他議員，
 *   將主旨議員插入第一位，保留非議員受文者。
 *
 * 新增的 <全銜> 格式：
...
export function autoFillRecipients(xmlDoc, dataRepo) {
  ...
  // ── 前置檢查：主旨必須含「議員」；「等議員提案」交由提案議員名單 modal 處理 ──
  if (!subjectText.includes('議員')) {
    return { skipped: true, reason: '主旨中未出現「議員」關鍵字，副本未自動更新' };
  }
  if (subjectText.includes('等議員提案')) {
    return { skipped: true, reason: '等議員提案模式：副本由提案議員名單視窗處理' };
  }
